...
import { createToken, publicKeyHex, futureTimestamp, pastTimestamp, TEST_LIBRARY_ID } from "./helpers.ts";
...
describe("domain checking", () => {
  describe("checkDomain", () => {
    ...
    test("allows wildcard subdomain match", async () => {
      ...
      expect((await validator.checkDomain(token, "deep.nested.example.com")).allowed).toBe(true);
    });
